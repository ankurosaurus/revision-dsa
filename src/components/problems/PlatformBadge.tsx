...
export const PlatformBadge: React.FC<PlatformBadgeProps> = ({
  ...
  switch (platform) {
    case 'leetcode':
      return (
        <span
          className={`inline-flex items-center font-medium rounded-md bg-surface text-paper-primary border border-surface-border ${sizeClasses}`}
        >
          <svg className="w-3 h-3 shrink-0 text-ochre" viewBox="0 0 24 24" fill="currentColor">
      ...
      return (
        ...
        >
          <div className="flex items-end gap-0.5 h-2.5 shrink-0">
            <span className="w-0.5 h-1.5 bg-paper-secondary rounded-t-xs" />
            <span className="w-0.5 h-2.5 bg-paper-primary rounded-t-xs" />
            <span className="w-0.5 h-2 bg-ochre rounded-t-xs" />
          </div>
